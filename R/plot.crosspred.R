plot.crosspred <- function(x, ptype, var = NULL, lag = NULL, ci = "area", ci.arg,
  ...
  if (ptype == "slices") {
    ...
    if (!is.null(lag)) {
      ...
      for (i in seq(along = lag)) {
        ...
        ci.list <- list(panel.first = call("fci",
          ci = ci, x = x$predvar,
          high = x$mathigh[, xlag[i]], low = x$matlow[, xlag[i]], ci.arg, plot.arg,
          noeff = noeff
        ))
        plot.arg <- modifyList(plot.arg, c(
          ci.list,
  ...
  if (ptype == "overall") {
    # SET DEFAULT VALUES IF NOT INCLUDED BY THE USER
    plot.arg <- list(
      type = "l", ylim = c(min(x$alllow), max(x$allhigh)),
  ...
  if (ptype == "contour") {
    ...
    filled.contour(
      x = x$predvar, y = seqlag(x$lag, x$bylag), z = x$matfit, col = col,
      levels = levels, ...
    )
  }
