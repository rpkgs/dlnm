thr <- function(x, thr.value = NULL, side = NULL, intercept = FALSE) {
  ...
  if (is.null(thr.value)) {
    thr.value <- median(x, na.rm = FALSE)
  } else {
  ...
  # TRANSFORMATION
  basis <- switch(side,
    h = as.matrix(pmax(x - thr.value, 0)),  # 高于阈值多少
    l = as.matrix(-pmin(x - thr.value, 0)), # 低于阈值多少
    d = cbind(-pmin(x - thr.value[1], 0), pmax(x - thr.value[2], 0)) # 低于, 高于
  )
